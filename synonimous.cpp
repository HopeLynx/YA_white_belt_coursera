...
Формат ввода
Сначала вводится количество запросов Q, затем Q строк с описаниями запросов. Гарантируется, что в каждом запросе CHECK и ADD слова word1 и word2 различны.
...
int main(){
    ...
    for(int i = 0; i < n; i++){
        ...
        if (s == "ADD"){
            ...
            string s1,s2; cin >> s1 >> s2;
            auto it = dict[s1].find(s2);
            print_Y_N(it != dict[s1].end());

        } else if (s == "COUNT"){

            string tmp; cin >> tmp;
            cout << dict[tmp].size() << endl;
